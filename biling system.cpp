...
void shopping :: add()
{
	...
	data.open("stock.txt", ios ::in) ; // open in read-mode
	
	if(! data)
	{
		data.open("stock.txt", ios :: app | ios :: out) ; //  open in append-mode
	...
	else
	{
		...
		{
			data.open("stock.txt", ios :: app | ios :: out) ;
			data << " " << pcode << " " << pname << " " << price << " " << dis << "\n" ;
			data.close() ;
		}
